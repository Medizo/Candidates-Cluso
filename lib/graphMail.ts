export const sendMsGraphEmail = async (
  ...
  if (!TENANT_ID || !CLIENT_ID || !CLIENT_SECRET || !SENDER_EMAIL) {
    return {
      sent: false,
      reason:
        "Microsoft Graph credentials are not fully configured in environment.",
    };
  }
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      return {
        sent: false,
        reason: `Failed to get access token: ${errorText}`,
      };
    }
    ...
    if (!mailResponse.ok) {
      ...
      return {
        sent: false,
        reason: `Failed to send email: ${mailResponse.status} ${errorText}`,
      };
    }
